  ...
  reglas: {
    ...
    cursosDisponibles: (cursosAprobados) => {
      return Object.keys(knowledgeBase.cursos).filter(curso => {
        return knowledgeBase.reglas.puedeTomar(curso, cursosAprobados) && 
...
class Chatbot {
  ...
  evaluarExpresion(expresion) {
    ...
    const exprNormalizada = expresion.replace(/ y /gi, ' AND ')
                                    .replace(/ o /gi, ' OR ')
                                    .replace(/ no /gi, ' NOT ')
                                    .replace(/\?/g, '')
                                    .trim();
    
    const partes = exprNormalizada.split(/(AND|OR|NOT)/).map(s => s.trim()).filter(s => s);
    ...
    for (const parte of partes) {
      if (parte === 'AND' || parte === 'OR' || parte === 'NOT') {
        ...
        let valor = this.cursosAprobados.includes(parte) || !!knowledgeBase.cursos[parte];
  ...
  procesarPregunta(pregunta) {
    ...
    try {
      ...
      if (/¿?Puedo tomar (.+)\??/i.test(pregunta) || 
        ...
        const curso = cursoInput ? knowledgeBase.reglas.buscarCursoPorNombre(cursoInput) : null;
        
        if (!curso) {
          const respuesta = "No reconozco ese curso. Los cursos disponibles son: " + 
            Object.keys(knowledgeBase.cursos).join(', ');
          this.historial.push(`Chatbot: ${respuesta}`);
          return respuesta;
        }
        
        if (pregunta.toLowerCase().includes('qué necesito') || 
          ...
          const respuesta = puedeTomar ? 
            ...
            `No puedes tomar ${curso}. Requisitos faltantes: ${knowledgeBase.cursos[curso].requisito || 'ninguno'}`;
          this.historial.push(`Chatbot: ${respuesta}`);
          return respuesta;
        }
      }
      ...
      else if (/Aprobé (.+)/i.test(pregunta) || 
               /He aprobado (.+)/i.test(pregunta) || 
               /Registra que pasé (.+)/i.test(pregunta) || 
               /Ya terminé (.+)/i.test(pregunta)) {
        const match = pregunta.match(/Aprobé (.+)|He aprobado (.+)|Registra que pasé (.+)|Ya terminé (.+)/i);
        const cursoInput = match ? (match[1] || match[2] || match[3] || match[4]) : null;
        const curso = cursoInput ? knowledgeBase.reglas.buscarCursoPorNombre(cursoInput) : null;
        
        if (!curso) {
          const respuesta = "No reconozco ese curso. Por favor especifica un curso válido.";
          this.historial.push(`Chatbot: ${respuesta}`);
          return respuesta;
        }
      ...
      else if (/¿?Cuál es el horario de (.+)\??/i.test(pregunta) || 
               /¿?Qué días se imparte (.+)\??/i.test(pregunta) || 
               /¿?A qué hora es (.+)\??/i.test(pregunta)) {
        const match = pregunta.match(/horario de (.+)|días se imparte (.+)|hora es (.+)/i);
      ...
      else if (/Evalúa (.+)/i.test(pregunta) || 
               /Verifica (.+)/i.test(pregunta)) {
        const expresion = pregunta.replace(/Evalúa /i, '')
                                 .replace(/Verifica /i, '')
                                 .replace(/\?/g, '')
                                 .trim();
      ...
      // 5. Preguntas combinadas (para probar inferencias)
        ...
        const cursoAprobado = cursoAprobadoInput ? knowledgeBase.reglas.buscarCursoPorNombre(cursoAprobadoInput) : null;
        const cursoDeseado = cursoDeseadoInput ? knowledgeBase.reglas.buscarCursoPorNombre(cursoDeseadoInput) : null;
        
        if (!cursoAprobado || !cursoDeseado) {
          const respuesta = "No entendí la pregunta. Por favor formula tu pregunta como: 'Si aprobé X, ¿puedo tomar Y?'";
          this.historial.push(`Chatbot: ${respuesta}`);
          return respuesta;
        }
        
        const tempAprobados = [...this.cursosAprobados, cursoAprobado];
        const puedeTomar = knowledgeBase.reglas.puedeTomar(cursoDeseado, tempAprobados);
        const respuesta = puedeTomar ? 
          `Sí, si apruebas ${cursoAprobado}, podrías tomar ${cursoDeseado}.` : 
          `No, incluso aprobando ${cursoAprobado}, no podrías tomar ${cursoDeseado}. Requisitos faltantes: ${knowledgeBase.cursos[cursoDeseado].requisito || 'ninguno'}`;
        this.historial.push(`Chatbot: ${respuesta}`);
        return respuesta;
      }
      else if (/¿Qué cursos puedo tomar si solo he aprobado (.+)\??/i.test(pregunta)) {
        const match = pregunta.match(/¿Qué cursos puedo tomar si solo he aprobado (.+)\??/i);
        const cursoAprobadoInput = match ? match[1] : null;
        const cursoAprobado = cursoAprobadoInput ? knowledgeBase.reglas.buscarCursoPorNombre(cursoAprobadoInput) : null;
        ...
        const disponibles = knowledgeBase.reglas.cursosDisponibles(tempAprobados);
      ...
      // 6. Manejo de errores
        ...
        const respuesta = "Por favor especifica qué curso has aprobado. Ejemplo: 'Aprobé Programación I'";
        this.historial.push(`Chatbot: ${respuesta}`);
        return respuesta;
      }
      else if (/Evalúa .* y .*/i.test(pregunta)) {
        const respuesta = "Para evaluar expresiones, usa 'AND' en lugar de 'y'. Ejemplo: 'Evalúa Programación I AND Matemáticas Discretas'";
        this.historial.push(`Chatbot: ${respuesta}`);
        return respuesta;
      }
      
      // 7. Preguntas complejas
      else if (/Necesito saber si puedo tomar (.+) habiendo aprobado solo (.+)/i.test(pregunta)) {
        const match = pregunta.match(/Necesito saber si puedo tomar (.+) habiendo aprobado solo (.+)/i);
        ...
        if (!cursoDeseado || !cursoAprobado) {
          const respuesta = "No entendí la pregunta. Por favor formula tu pregunta como: 'Necesito saber si puedo tomar X habiendo aprobado solo Y'";
          this.historial.push(`Chatbot: ${respuesta}`);
          return respuesta;
        }
        ...
        const respuesta = puedeTomar ? 
          ...
          `No, con solo ${cursoAprobado} aprobado, no podrías tomar ${cursoDeseado}. Requisitos faltantes: ${knowledgeBase.cursos[cursoDeseado].requisito || 'ninguno'}`;
        this.historial.push(`Chatbot: ${respuesta}`);
        return respuesta;
      }
...
- Preguntas condicionales: "Si aprobé Programación I, ¿puedo tomar Bases de Datos?"`;
      ...
      const respuesta = "Ocurrió un error al procesar tu pregunta. Por favor intenta de nuevo.";
      this.historial.push(`Chatbot: ${respuesta}`);
      return respuesta;
    }
  }
...
function ejecutarTodasLasPruebas() {
  ...
  console.log(bot.procesarPregunta("¿Qué necesito para tomar Inteligencia Artificial?"));
  console.log(bot.procesarPregunta("¿Puedo tomar Bases de Datos sin haber aprobado Programación I?"));
  console.log(bot.procesarPregunta("¿Cuáles son los requisitos para Matemáticas Discretas?"));
  ...
  console.log(bot.procesarPregunta("¿Cuántos créditos vale Inteligencia Artificial?"));
  console.log(bot.procesarPregunta("¿A qué hora es Ingeniería de Software?"));
  
  console.log("\n4. Evaluación de expresiones lógicas:");
  console.log(bot.procesarPregunta("Evalúa Programación I AND Matemáticas Discretas"));
  console.log(bot.procesarPregunta("Verifica Programación I OR Bases de Datos"));
  console.log(bot.procesarPregunta("Evalúa NOT Programación I"));
  console.log(bot.procesarPregunta("Verifica Inteligencia Artificial AND NOT Matemáticas Discretas"));
  console.log(bot.procesarPregunta("Evalúa Programación I AND Bases de Datos AND Ingeniería de Software"));
  ...
  console.log(bot.procesarPregunta("Evalúa Programación I Y Matemáticas Discretas"));
  console.log(bot.procesarPregunta("Horario de Física Cuántica"));
  console.log(bot.procesarPregunta("Aprobé"));
  
  console.log("\n7. Preguntas complejas:");
  console.log(bot.procesarPregunta("Si aprobé Programación I, ¿puedo tomar Bases de Datos?"));
  console.log(bot.procesarPregunta("¿Qué cursos puedo tomar si solo he aprobado Programación I?"));
  console.log(bot.procesarPregunta("Necesito saber si puedo tomar Inteligencia Artificial habiendo aprobado solo Matemáticas Discretas"));
